...
¡Bienvenido a **SportEventHub**! Este proyecto es la API backend para una plataforma integral de gestión de eventos deportivos, inscripciones y control de tickets/cupos en tiempo real. Conecta a atletas y participantes con clubes, entrenadores y organizadores de eventos (maratones, torneos, partidos, competencias y clínicas deportivas).

Este desarrollo representa la **Entrega Final de Backend II**, implementando una arquitectura profesional formal por capas (**DAO, Repository, Services, Controllers, DTOs**), autenticación centralizada mediante **Passport.js**, **JWT en cookies HttpOnly**, Hashing con `bcrypt`, **Control de Acceso Basado en Roles (RBAC)**, control estricto de cupos en tiempo real, notificaciones vía **Nodemailer** y **Tests Automatizados**.
...
```text
               ...
               ┌─────────────────────────────────────────┐
               │               Controllers               │
               │  (Solo orquesta req/res y formatea DTO) │
               └────────────────────┬────────────────────┘
                                    │ Llama al Service
                                    ▼
               ┌─────────────────────────────────────────┐
               │                Services                 │
               │   (Lógica de negocio, cupos, fechas     │
               │       vigentes, permisos y emails)      │
               └────────────────────┬────────────────────┘
                                    │ Consume Repository
                                    ▼
               ┌─────────────────────────────────────────┐
               │              Repositories               │
               │   (Abstracción de dominio de datos)     │
               └────────────────────┬────────────────────┘
                                    │ Consume DAO
                                    ▼
               ┌─────────────────────────────────────────┐
               │                  DAOs                   │
               │   (Única capa que interactúa con ODM)   │
               └────────────────────┬────────────────────┘
                                    │ Consultas Mongoose
                                    ▼
               ┌─────────────────────────────────────────┐
...
1. **DAOs (`src/dao/`)**: Son los **únicos archivos que importan modelos de Mongoose directamente**. Exponen operaciones atómicas de lectura, escritura y agregación (`findById`, `findOne`, `create`, `update`, `countDocuments`, `aggregate`).
2. **Repositories (`src/repositories/`)**: Abstraen la persistencia hacia el dominio. **No importan modelos de Mongoose**; consumen exclusivamente sus DAOs correspondientes y exponen métodos orientados a la entidad (`findByEmail`, `getPaginatedEvents`, `getActiveUserTicket`, `getOccupiedCapacity`, `cancelTicket`).
3. **Services (`src/services/`)**: Concentran **todas las reglas de negocio**. Consumen exclusivamente repositorios (nunca DAOs ni modelos directamente). Gestionan validación de cupos, control de fechas vigentes (`event.date > Date.now()`), no duplicados, verificación de permisos y despacho de correos electrónicos.
4. **Controllers (`src/controllers/`)**: Solo coordinan request/response. Extraen parámetros (`params`, `query`, `body`), invocan al servicio correspondiente y estructuran la respuesta aplicando los **DTOs**. No contienen lógica de negocio ni importan modelos.
5. **DTOs (`src/dto/`)**: Garantizan la seguridad en todas las respuestas:
   - **`UserDTO`**: Oculta de forma estricta cualquier campo `password` (incluso el hash) en todas las salidas (`/current`, `/register`, `/api/users`).
   - **`EventDTO`**: Normaliza los campos del evento y filtra datos sensibles del organizador si viene poblado.
...
| **Inscribirse a un evento (Ticket)** | `POST /api/events/:eid/tickets` | Cualquier usuario autenticado | Valida fecha futura, evento publicado, no duplicados y cupos disponibles. Envía email de confirmación. |
...
| **Cerrar sesión** | `POST /api/sessions/logout` | Público / Autenticado | Elimina la cookie `currentUser`. |
| **Ver todos los usuarios** | `GET /api/users` | `admin` | Exclusivo para administradores. `403` para otros roles. |

---

## 👥 Usuarios de Prueba y Cómo Crearlos

Para facilitar la evaluación de los roles y permisos, puedes utilizar o registrar los siguientes perfiles:

### 1. Usuario Regular (`user`)
Se crea directamente mediante el endpoint público de registro:
- **Email**: `deportista@deportes.com`
- **Password**: `Password123!`
- **Rol asignado**: `user` (automático por defecto)

```bash
curl -X POST http://localhost:8080/api/sessions/register \
  -H "Content-Type: application/json" \
  -d '{"first_name":"Mateo","last_name":"Gómez","email":"deportista@deportes.com","password":"Password123!"}'
```

### 2. Organizador de Eventos (`organizer`)
Por seguridad, el registro público no permite setear roles elevados en el body. Para asignar el rol `organizer`:
- Registrar el usuario normalmente.
- En MongoDB Atlas o MongoDB Compass, actualizar el campo `role` del documento a `"organizer"`.
- **Credenciales sugeridas**:
  - **Email**: `club@central.com`
  - **Password**: `Password123!`
  - **Rol**: `organizer`

### 3. Administrador (`admin`)
- En la base de datos, actualizar el campo `role` del documento a `"admin"`.
- **Credenciales sugeridas**:
...
El proyecto incluye en la raíz el archivo listo para importar en Postman:
👉 **[`SportEventHub.postman_collection.json`](./SportEventHub.postman_collection.json)**

### Cómo Importar y Usar:
1. Abre Postman y haz clic en **Import**.
2. Selecciona el archivo `SportEventHub.postman_collection.json` de la carpeta del proyecto.
3. La colección ya incluye la variable de entorno `baseUrl = http://localhost:8080`.
4. Las cookies de sesión (`currentUser`) son gestionadas automáticamente por Postman tras invocar la petición de **Login**.
5. Las carpetas están organizadas en orden de flujo:
   - `1. Sesiones y Autenticación` (Register, Login, Current, Logout)
   - `2. Eventos Deportivos` (Listado con filtros, Detalle, Crear evento, Modificar, Cambiar estado)
   - `3. Tickets e Inscripciones` (Inscribirse, Mis tickets, Tickets del evento, Cancelar ticket)
   - `4. Administración de Usuarios` (Listar usuarios solo admin)
...
curl -X POST http://localhost:8080/api/sessions/login \
  -H "Content-Type: application/json" \
  -d '{"email":"deportista@deportes.com","password":"Password123!"}' \
  -c cookies.txt
```
...
├── SportEventHub.postman_collection.json # Colección oficial para Postman
...
## 🧪 Ejecución de Tests Automatizados (`npm test`)

Para ejecutar la suite de pruebas que valida de forma automatizada los **10 flujos críticos exigidos en la entrega final**:

```bash
npm test
```

### Resumen de Pruebas de la Entrega Final:

| # | Flujo Evaluado | Resultado Esperado | Estado |
| :---: | :--- | :---: | :---: |
| **1** | Registro → Login → `/current` (200) → Logout → `/current` | Devuelve `401` tras logout | ✅ Superado |
| **2** | `user` intenta crear evento | `403 Forbidden` | ✅ Superado |
| **3** | `organizer` crea evento → `user` se inscribe → email recibido | Cupo descontado (201) | ✅ Superado |
| **4** | `user` intenta inscribirse nuevamente al mismo evento | `409 Conflict` (duplicado activo) | ✅ Superado |
| **5** | `user` intenta inscribirse a evento sin cupo suficiente | `400 Bad Request` | ✅ Superado |
| **6** | `user` cancela su ticket → cupo liberado | Nueva inscripción funciona (201) | ✅ Superado |
| **7** | `organizer` intenta modificar evento ajeno | `403 Forbidden` | ✅ Superado |
| **8** | `admin` modifica evento de otro organizador | `200 OK` | ✅ Superado |
| **9** | Respuestas de usuario, evento y ticket pasan por DTO | **NUNCA contienen password** | ✅ Superado |
| **10** | Listado con `?status=published&page=2&limit=5` | Paginado `{ data, page, limit, total, totalPages }` | ✅ Superado |
